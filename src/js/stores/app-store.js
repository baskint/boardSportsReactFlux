...
for (let i=1; i< 9; i++) {
  _catalog.push({
    'id': 'Board' + i,
    'title': 'Board # ' + i,
    'summary': 'a great board',
    'description': 'rides versatile',
    'cost' : i
  });
}
...
const AppStore = Object.assign(EventEmitter.prototype, {
  ...
  dispatcherIndex: register( function( action ){
    switch(action.actionType){
      case AppConstants.ADD_BOARD:
          _addItem( action.item );
          break;
      case AppConstants.REMOVE_BOARD:
          _removeItem( action.item );
          break;

      case AppConstants.ADD_SESSION:
          _increaseItem( action.item );
          break;

      case AppConstants.REMOVE_SESSION:
